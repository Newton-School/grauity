# NSButton Migration Script (v3.0.0)

This JSCodeShift script does the automated migration for NSButton components to v3.0.0.
...
module.exports = function transformer(fileInfo, api) {
    ...
    // Process JSX elements: <NSButton ... />
    root.find(j.JSXOpeningElement, { name: { name: 'NSButton' } }).forEach(
        (path) => {
    ...
    // Process styled-components inheritance: styled(NSButton)
    root.find(j.CallExpression, {
        callee: { name: 'styled' },
        arguments: [{ name: 'NSButton' }],
    }).forEach((path) => {
        const commentText =
            ' TODO: Manual review: NSButton inheritance via styled-components. Verify variant prop mapping if applicable.';
