...
_**Note**_: If passing a custom React component to the `as` prop, ensure the following:
- The component MUST ACCEPT `className` prop to apply the necessary styles, otherwise theming will NOT work!
- The component MUST NOT ACCEPT props with keys from the following: `applyTheme`, `invert`, `as`, as these props are reserved for `ThemeScope` functionality.
