...
- `error` is an object containing error messages for each field. Keys will be field names and value will error message.

```typescript
const MyForm = () => {
    const { formData, formRenderer, validate, submit, changeFormData, error } = useNSForm({
        formConfig,
        rowStyles = {},
        whenToValidate = null,
        isMobileView = false,
